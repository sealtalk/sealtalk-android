...
public interface FriendDao {
    ...
    @Query(
            "SELECT user.id,user.name,user.portrait_uri FROM black_list INNER JOIN user ON black_list.id = user.id WHERE black_list.id=:userId")
    LiveData<UserSimpleInfo> getUserInBlackList(String userId);

    @Query(
            "SELECT user.id,user.name,user.portrait_uri FROM black_list INNER JOIN user ON black_list.id = user.id")
    ...
    @Query(
            "SELECT friend.id as id ,alias,portrait_uri,name,region,phone_number,friend_status,message,updateAt,alias_spelling, name_spelling,order_spelling "
                    ...
                    + ")"
                    + "order by user.order_spelling")
    LiveData<List<FriendShipInfo>> searchFriendsExcludeGroup(
            String excludeGroupId, String matchSearch);

    @Query(
            "SELECT group_member.user_id as id ,alias,portrait_uri,name,region,phone_number,friend_status,alias_spelling, name_spelling,order_spelling,message, updateAt, nickname, nickname_spelling "
                    + "FROM group_member "
                    + "left join user "
    ...
    @Query(
            "SELECT group_member.user_id as id ,alias,portrait_uri,name,region,phone_number,friend_status,alias_spelling, name_spelling,order_spelling,message, updateAt, nickname, nickname_spelling "
                    ...
                    + ")"
                    + "order by user.order_spelling")
    LiveData<List<FriendShipInfo>> searchFriendsIncludeGroup(
    ...
    @Query(
            "SELECT phone_contact.user_id, phone_contact.phone_number, phone_contact.is_friend,phone_contact.contact_name, user.name, user.portrait_uri, user.name_spelling, user.st_account FROM phone_contact "
                    + "LEFT JOIN user "
