...
public class QRCodeManager {
        ...
        Uri sealGroupUri =
                new Uri.Builder()
                        ...
                        .appendQueryParameter(
                                QRCodeConstant.SealTalk.GROUP_QUERY_SHARED_USER_ID, sharedUserId)
                        .build();
        Uri fullUri =
                baseUri.buildUpon()
                        .appendQueryParameter(
                                QRCodeConstant.BASE_URL_QUERY_CONTENT, sealGroupUri.toString())
                        .build();
     ...
     * @return
     */
    public LiveData<Resource<Bitmap>> getGroupQRCode(
            String groupId, String sharedUserId, int width, int height) {
                        ...
                        new Runnable() {
                            ...
                            public void run() {
                                String qrCodeContent =
                                        generateGroupQRCodeContent(groupId, sharedUserId);
                                Bitmap bitmap =
                                        QRCodeUtils.generateImage(
                                                qrCodeContent, width, height, null);
    ...
    public QRCodeResult getQRCodeType(String qrCodeStr) {
        ...
        if (uri.getScheme() != null
                && QRCodeConstant.SealTalk.SCHEME.equals(uri.getScheme().toLowerCase())) {
            sealUri = uri;
        } else {
            ...
            if (!TextUtils.isEmpty(content)) {
                ...
                if (contentUri.getScheme() != null
                        && QRCodeConstant.SealTalk.SCHEME.equals(
                                contentUri.getScheme().toLowerCase())) {
                    sealUri = contentUri;
                }
            }
        }
        ...
        if (sealUri != null) {
            ...
            if (QRCodeConstant.SealTalk.AUTHORITY_GROUP.equals(authority)) {
                ...
                if (QRCodeConstant.SealTalk.GROUP_PATH_JOIN.equals(path)) {
                    String groupId =
                            sealUri.getQueryParameter(QRCodeConstant.SealTalk.GROUP_QUERY_GROUP_ID);
                    String sharedId =
                            sealUri.getQueryParameter(
                                    QRCodeConstant.SealTalk.GROUP_QUERY_SHARED_USER_ID);
                ...
                if (QRCodeConstant.SealTalk.USER_PATH_INFO.equals(path)) {
                    String userId =
                            sealUri.getQueryParameter(QRCodeConstant.SealTalk.USER_QUERY_USER_ID);
