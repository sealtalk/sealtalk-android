...
public class GroupListActivity extends TitleAndSearchBaseActivity
        implements OnGroupItemClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        searchGroupByNameFragment.setOnSearchResultListener(
                new SearchGroupByNameFragment.SearchResultListener() {
                    @Override
                    public void onSearchResult(String lastKeyWord, List<SearchModel> searchModels) {
                        if (TextUtils.isEmpty(lastKeyWord)
                                && (searchModels == null || searchModels.size() == 0)) {
