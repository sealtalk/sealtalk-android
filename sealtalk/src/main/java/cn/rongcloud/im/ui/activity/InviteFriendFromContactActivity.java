...
public class InviteFriendFromContactActivity extends TitleAndSearchBaseActivity {
    ...
    private void initFragment() {
        ...
        contactFragment =
                (InviteFriendFromContactFragment)
                        supportFragmentManager.findFragmentByTag(
                                InviteFriendFromContactFragment.class.getSimpleName());
        if (contactFragment == null) {
            contactFragment = new InviteFriendFromContactFragment();
        }

        // 设置选择通讯录监听
        contactFragment.setOnContactSelectedListener(
                new InviteFriendFromContactFragment.OnContactSelectedListener() {
                    @Override
    ...
    private void inviteSelectedContacts() {
        if (contactFragment != null) {
            List<SimplePhoneContactInfo> checkedContactInfo =
                    contactFragment.getCheckedContactInfo();
