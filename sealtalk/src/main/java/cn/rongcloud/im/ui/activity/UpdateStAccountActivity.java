...
public class UpdateStAccountActivity extends TitleBaseActivity {
    ...
    private void initView() {
        ...
        // 设置 hint
        SpannableString ss =
                new SpannableString(getString(R.string.seal_update_staccout_edittext_hint));
        ...
        updateSAccountCet.setClearDrawable(
                getResources().getDrawable(R.drawable.seal_st_account_delete));
        updateSAccountCet.addTextChangedListener(
                new TextWatcher() {
                    @Override
                            ...
                            public void onChanged(Resource<UserInfo> userInfoResource) {
                                if (userInfoResource.data != null) {
                                    ...
                                    ImageLoaderUtils.displayUserPortraitImage(
                                            info.getPortraitUri(),
                                            uivUserInfo.getHeaderImageView());
                                }
                            }
                        });
        userInfoViewModel
                .getSetStAccountResult()
                .observe(
                        this,
    ...
    private void showSaveDialog(int type) {
        ...
        switch (type) {
            case SAVECONFIRM:
                ...
                StringBuffer buffer =
                        new StringBuffer(getString(R.string.seal_staccount_dialog_content));
                int i = buffer.indexOf("@");
                buffer.replace(i, i + 1, stAccout);
                SpannableString spannableString = new SpannableString(buffer.toString());
                spannableString.setSpan(
                        new ForegroundColorSpan(
                                getResources().getColor(R.color.default_clickable_text)),
                        i,
                        i + stAccout.length(),
                ...
                break;
            case FORMATERROR:
                builder.setTitleMessage(
                        getString(R.string.seal_staccount_dialog_title_format_error));
        ...
        builder.setDialogButtonClickListener(
                new StAccountDialog.OnDialogButtonClickListener() {
                    @Override
