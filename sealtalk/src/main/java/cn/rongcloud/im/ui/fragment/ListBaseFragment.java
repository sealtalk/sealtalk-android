...
/**
 * 此 Fragment 是列表展示的半成品。 继承于 {@link ListWithSidebarBaseFragment}。 在 ListWithSidebarBaseFragment
 * 基础上添加了 ViewModel 获取数据并刷新列表的逻辑。
 *
 * <p>Fragment 中有两个抽象方法。 {@link #getListAdapter()} 是需要子类返回的 Adapter （ ListWithSideBarBaseAdapter 或
 * 继承 ListWithSideBarBaseAdapter）。 另一个方法为 {@link #createViewModel()} . 子类需要实现此方法并返回 {@link
 * CommonListBaseViewModel} 的子类。
 *
 * <p>可通过继承Fragment 类并实现其抽象方法， 来进行自定义数据获取的逻辑部分和 Adapter 展示部分。
 *
...
public abstract class ListBaseFragment extends ListWithSidebarBaseFragment {
    ...
    protected void onInitViewModel() {
        ...
        viewModel
                ...
                .observe(
                        this,
                        new Observer<List<ListItemModel>>() {
                            @Override
    ...
    /**
     * 创建 viewmodel. 可以进行继承 ConversationListBaseViewModel 并实现获取数据的方法。 来实现自己的数据加载
     *
     * @return
     */
