...
/**
 * 此 Fragment 带有 RecyclerView 和 SideBar 功能控件。 继承此 Fragment 后即可使用上面两个功能控件。 当您不想使用 SideBar 控件时，可通过复写
 * {@link #isUseSideBar()} 方法并返回 false， 默认为 true。
 *
 * <p>此 Fragment 有一个 {@link #getAdapter()} 的抽象方法。 子类需要实现此方法， 并返回 {@link ListWithSideBarBaseAdapter}
 * 或及其子类。 可更具自己的需求去实现 adapter 的逻辑。
 *
 * @see ListWithSideBarBaseAdapter
 */
public abstract class ListWithSidebarBaseFragment extends BaseFragment
    ...
    /**
     * 设置是否使用显示侧边栏。 默认返回 true。 如果不想使用， 可复写此方法返回 false
     *
