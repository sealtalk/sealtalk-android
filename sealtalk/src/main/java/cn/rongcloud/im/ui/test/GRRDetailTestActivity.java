        ...
        receiptViewModel.getGroupMemberList().observe(this, new Observer<Resource<List<GroupMember>>>() {
            ...
            public void onChanged(Resource<List<GroupMember>> resource) {
                if (resource.data != null) {
                    ReadReceiptV2Manager.getInstance().getGroupMessageReaderList(targetMessage, new ReadReceiptV2Manager.IGetMessageReadUserListCallback() {
                        @Override
                        public void onSuccess(int members, List<GroupMessageReader> readers) {
                            RongIMClient.getInstance().getMessageByUid(targetMessage.getUId(), new RongIMClient.ResultCallback<Message>() {
                                @Override
