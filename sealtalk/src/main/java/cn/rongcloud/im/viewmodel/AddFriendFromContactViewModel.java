...
public class AddFriendFromContactViewModel extends CommonListBaseViewModel {
    ...
    public void loadData() {
        conversationLiveData.addSource(
                friendTask.getPhoneContactInfo(),
        ...
        // 搜索结果
        LiveData<List<PhoneContactInfo>> searchResult =
                Transformations.switchMap(
                        keywordLiveData, keyword -> friendTask.searchPhoneContactInfo(keyword));
        conversationLiveData.addSource(
                searchResult,
                new Observer<List<PhoneContactInfo>>() {
                    @Override
    ...
    protected ListItemModel createPhoneContact(PhoneContactInfo info) {
        ...
        ListItemModel.ItemView itemView =
                new ListItemModel.ItemView(
                        R.layout.add_friend_item_from_contact,
                        ListItemModel.ItemView.Type.FRIEND,
                        AddFriendFromContactItemViewHolder.class);
        ListItemModel<PhoneContactInfo> model =
                new ListItemModel<>(info.getUserId(), name, info, itemView);
