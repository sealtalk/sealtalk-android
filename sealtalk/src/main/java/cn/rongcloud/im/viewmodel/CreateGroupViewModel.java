...
public class CreateGroupViewModel extends AndroidViewModel {
    ...
    public void createGroup(String groupName, Uri groupPortrait, List<String> memberIdList) {
        LiveData<Resource<GroupResult>> createGroupResource =
                groupTask.createGroup(groupName, memberIdList);
    ...
    private void nextToUploadPortraitResult(GroupResult groupResult, Uri groupPortrait) {
        // 进行上传群组头像
        LiveData<Resource<Void>> uploadResource =
                groupTask.uploadAndSetGroupPortrait(groupResult.id, groupPortrait);
        createGroupResult.addSource(
                uploadResource,
                resource -> {
